"""Fit a long lane section with adaptive local models in metric Cartesian X/Z.
...
from scripts.fit_first5_two_curves import parse_projection  # noqa: E402
...
MODEL_COLOURS = {
    "parametric_polynomial": "#2b6cb0",
    "parametric_cubic_bspline": "#dd6b20",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Adaptive piecewise left/right lane fitting directly in pose-aligned "
            "metric Cartesian X/Z."
        )
    )
...
def select_transition_model(
    ...
    """Select from held-out RMSE, preferring the simpler model inside a tie."""
    ...
    for model, metrics in metrics_by_model.items():
        fold_count = int(metrics.get("heldout_fold_count", 0))
        rmse = float(metrics.get("rmse_m", float("nan")))
        if model in available and fold_count > 0 and np.isfinite(rmse):
            valid[model] = rmse
    if polynomial in valid and spline in valid:
        if valid[polynomial] <= valid[spline] + tie_m:
            return (
                polynomial,
                "transition comparison: polynomial held-out RMSE is lower or within the registered simplicity tie",
            )
        return spline, "transition comparison: B-spline has lower held-out RMSE beyond the registered tie"
    if polynomial in valid:
        return polynomial, "transition comparison: only polynomial has a valid held-out RMSE"
    if spline in valid:
        return spline, "transition comparison: only B-spline has a valid held-out RMSE"
    if polynomial in available:
        return polynomial, "transition fallback: no valid held-out RMSE, so choose the simpler polynomial"
...
def chord_parameter(points_xz: np.ndarray) -> np.ndarray:
    points = np.asarray(points_xz, dtype=np.float64).reshape(-1, 2)
    cumulative = np.concatenate(
        [[0.0], np.cumsum(np.linalg.norm(np.diff(points, axis=0), axis=1))]
    )
    if cumulative[-1] <= 1e-8:
        raise ValueError("Cannot order a zero-length curve.")
...
def fit_parametric_bspline(
    ...
    parameter = chord_parameter(aggregate_xz)
    degree = min(3, len(aggregate_xz) - 1)
    weights = np.ones(len(parameter), dtype=np.float64)
    tck = None
    residuals = np.zeros(len(parameter), dtype=np.float64)
    for _ in range(max(1, irls_iterations)):
        smoothing = max(smoothing_per_point_m2 * len(parameter), 1e-12)
        tck, _ = splprep(
            [aggregate_xz[:, 0], aggregate_xz[:, 1]],
            u=parameter,
            # FITPACK squares its supplied weights in the objective.  Passing
            # the square root applies the intended Huber residual weights.
            w=np.sqrt(weights),
            k=degree,
            s=smoothing,
        )
        predicted = np.column_stack(splev(parameter, tck))
        residuals = np.linalg.norm(aggregate_xz - predicted, axis=1)
        weights = huber_weights(residuals, huber_delta_m)
    assert tck is not None
    sample_parameter = np.linspace(0.0, 1.0, curve_samples)
    curve = np.column_stack(splev(sample_parameter, tck))
    knots, coefficients, spline_degree = tck
    return ParametricFit(
        model="parametric_cubic_bspline",
        ...
        residuals_m=residuals,
        coefficients={
            "knots": np.asarray(knots).tolist(),
            "x": np.asarray(coefficients[0]).tolist(),
            "z": np.asarray(coefficients[1]).tolist(),
        },
        huber_delta_m=huber_delta_m,
        irls_iterations=irls_iterations,
    )
...
def fit_model(
    ...
    if model == "parametric_cubic_bspline":
        return fit_parametric_bspline(
            aggregate_xz,
            support_frames,
            args.smoothing_per_point_m2,
            args.huber_delta_m,
...
def plot_results(
    ...
    ax = axes[2]
    plotted = False
    for model, marker in (("parametric_polynomial", "o"), ("parametric_cubic_bspline", "s")):
        rows = [
            row
    ...
    figure.suptitle("Adaptive piecewise lane fitting in metric Cartesian X/Z", fontsize=14)
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    for spec in windows:
        ...
        window_row: dict[str, object] = {
            ...
            "selected_model": (
                fixed_model if fixed_model is not None else "comparison_by_side"
            ),
            "selected_model_short": (
                f"poly{args.polynomial_degree}"
                if fixed_model == "parametric_polynomial"
                else (
                    "B-spline"
                    if fixed_model == "parametric_cubic_bspline"
                    else "compare L/R"
                )
            ),
        }
        ...
        for side_index, side in enumerate(SIDES):
            ...
            for model in ("parametric_polynomial", "parametric_cubic_bspline"):
                try:
            ...
            if fixed_model is not None:
                ...
                selection_reason = (
                    "fixed policy: straight window uses the registered polynomial"
                    if classification == "straight"
    ...
    plot_results(
        args.output_dir / "adaptive_piecewise_xz_overview.png",
        route_common,
        window_results,
        blended_segments,
        window_rows,
    ...
    result_document: dict[str, object] = {
        ...
        "geometry_policy": (
            ...
            "or X(u), Z(u) fitting -> common-reference X/Z"
        ),
        "parameter_policy": (
            "q and u are dimensionless curve-ordering variables, not spatial coordinates"
        ),
        ...
        "warnings": warnings,
        "outputs": {
            "overview_png": "adaptive_piecewise_xz_overview.png",
